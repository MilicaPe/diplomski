...
public interface ResultRepository extends JpaRepository<Result, Long> {
    ...
    @Query("select e from Result e where e.detected=?1 and e.time>=?2 and e.time<=?3 order by e.time")
    List<Result> findAllByDetectedAndTime(DetectionType d, LocalDateTime start, LocalDateTime end);
